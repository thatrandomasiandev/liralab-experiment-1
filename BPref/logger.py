...
COMMON_TRAIN_FORMAT = [
    ...
    ('window_id', 'WID', 'int'),
    ('step_in_window', 'SIW', 'int'),
    ('reward_acc_train', 'RAT', 'float'),
    ('reward_acc_heldout', 'RAH', 'float'),
    ('reward_acc_onpolicy', 'RAO', 'float'),
    ('reward_acc_fixed_ref', 'RAF', 'float'),
    ('duration', 'D', 'time'),
...
class MetersGroup(object):
    ...
    def _dump_to_csv(self, data):
        if self._csv_writer is None:
            # Seed columns from console format so later keys don't blow up DictWriter.
            format_keys = [key for key, _, _ in self._formating]
            self._fieldnames = sorted(set(list(data.keys()) + format_keys + ['step']))
            ...
            # Ignore unexpected keys (science metrics also live in dedicated CSVs).
            pass
